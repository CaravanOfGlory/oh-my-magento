...
**注意**：请使用已发布的包名和二进制名 `oh-my-magento`。在 `opencode.json` 中，兼容性层现在优先使用插件入口 `oh-my-openagent`，而旧的 `oh-my-magento` 条目仍会加载并显示警告。插件配置文件通常仍使用 `oh-my-magento.json` 或 `oh-my-magento.jsonc`，在过渡期间新旧两种文件名都会被识别。
