...
OpenCode plugin (npm: `oh-my-magento`) extending Claude Code with multi-agent orchestration, 52 lifecycle hooks, 26 tools, skill/command/MCP systems, Hashline edit tool, IntentGate classifier, and Claude Code compatibility. ~1600 TypeScript source files. Dual-published as `oh-my-magento` + `oh-my-openagent` during transition.
...
│   ├── shared/               # 170+ utility files (barrel-exported, logger → /tmp/oh-my-magento.log)
...
- **Dual package**: `oh-my-magento` + `oh-my-openagent` published simultaneously (transition period)
...
| publish.yml | manual dispatch | Version bump, dual npm publish (oh-my-magento + oh-my-openagent), platform binaries, GitHub release |
