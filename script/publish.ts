...
async function getContributors(previous: string): Promise<string[]> {
  ...
  const team = ["actions-user", "github-actions[bot]", "caravanglory"]

  try {
    const compare =
      await $`gh api "/repos/caravanglory/oh-my-openagent/compare/v${previous}...HEAD" --jq '.commits[] | {login: .author.login, message: .commit.message}'`.text()
