...
Model capabilities are models.dev-backed, with a refreshable cache and capability diagnostics. Use `bunx oh-my-magento refresh-model-capabilities` to update the cache, or configure `model_capabilities.auto_refresh_on_start` to refresh at startup.

To see which models your agents will actually use, run `bunx oh-my-magento doctor`. This shows effective model resolution based on your current authentication and config.
