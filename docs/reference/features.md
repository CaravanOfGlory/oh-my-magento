...
Customize agent models, prompts, and permissions in `oh-my-magento.jsonc`.
...
You can define custom categories in your plugin config file. During the rename transition, both `oh-my-magento.json[c]` and legacy `oh-my-magento.json[c]` basenames are recognized.
...
Run `bunx oh-my-magento doctor` to see capability diagnostics including:
