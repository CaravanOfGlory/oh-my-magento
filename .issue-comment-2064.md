...
When a user runs `oh-my-magento install` (even just to update settings), the installer:
...
- **Trigger:** Running `oh-my-magento install` (even for unrelated updates)

### Workaround (Until Fix)

Backup your config before running install:
```bash
cp ~/.config/opencode/oh-my-magento.jsonc ~/.config/opencode/oh-my-magento.jsonc.backup
```
