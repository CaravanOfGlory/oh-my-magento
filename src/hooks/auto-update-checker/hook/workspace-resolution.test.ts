...
mock.module("../../../shared", () => ({
  ...
  getOpenCodeConfigPaths: () => ({
    ...
    omoConfig: join(TEST_CONFIG_DIR, "oh-my-magento.json"),
  }),
  getOpenCodeConfigDir: () => TEST_CONFIG_DIR,
}))

// Mock constants BEFORE importing the module
const ORIGINAL_PACKAGE_NAME = "oh-my-magento"
...
mock.module("../../../shared/data-path", () => ({
  ...
  getOmoOpenCodeCacheDir: () => join(TEST_DIR, "oh-my-magento"),
  getOpenCodeCacheDir: () => TEST_CACHE_DIR,
}))
...
describe("workspace resolution", () => {
  ...
  describe("#given config-dir install exists but cache-dir does not", () => {
    it("installs to config-dir, not cache-dir", async () => {
      ...
      mkdirSync(join(TEST_CONFIG_DIR, "node_modules", "oh-my-magento"), { recursive: true })
      writeFileSync(
        join(TEST_CONFIG_DIR, "package.json"),
        JSON.stringify({ dependencies: { "oh-my-magento": "3.4.0" } }, null, 2)
      )
      writeFileSync(
        join(TEST_CONFIG_DIR, "node_modules", "oh-my-magento", "package.json"),
        JSON.stringify({ name: "oh-my-magento", version: "3.4.0" }, null, 2)
      )
  ...
  describe("#given both config-dir and cache-dir exist", () => {
    it("prefers config-dir over cache-dir", async () => {
      ...
      mkdirSync(join(TEST_CACHE_DIR, "node_modules", "oh-my-magento"), { recursive: true })
      writeFileSync(
        join(TEST_CACHE_DIR, "package.json"),
        JSON.stringify({ dependencies: { "oh-my-magento": "3.4.0" } }, null, 2)
      )
      writeFileSync(
        join(TEST_CACHE_DIR, "node_modules", "oh-my-magento", "package.json"),
        JSON.stringify({ name: "oh-my-magento", version: "3.4.0" }, null, 2)
      )
