...
const REPO = "caravanglory/go-claude-code-comment-checker"
...
/**
 * Get the cache directory for oh-my-magento binaries.
...
export function getCacheDir(): string {
  if (process.platform === "win32") {
    ...
    return join(base, "oh-my-magento", "bin")
  }
...
export async function downloadCommentChecker(): Promise<string | null> {
  ...
  log(`[oh-my-magento] Downloading comment-checker binary...`)
  ...
  try {
    ...
    log(`[oh-my-magento] comment-checker binary ready.`)
    
    return binaryPath
    
  } catch (err) {
    debugLog(`Failed to download: ${err}`)
    log(`[oh-my-magento] Failed to download comment-checker: ${err instanceof Error ? err.message : err}`)
    log(`[oh-my-magento] Comment checking disabled.`)
    return null
  }
}
