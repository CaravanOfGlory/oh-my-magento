...
async function importFreshCacheModule(): Promise<typeof import("../auto-update-checker/cache")> {
  mock.module("../auto-update-checker/constants", () => ({
    ...
    INSTALLED_PACKAGE_JSON: join(TEST_OPENCODE_CACHE_DIR, "node_modules", "oh-my-magento", "package.json"),
...
function resetTestCache(): void {
  ...
  mkdirSync(join(TEST_OPENCODE_CACHE_DIR, "node_modules", "oh-my-magento"), { recursive: true })
  writeFileSync(
    join(TEST_OPENCODE_CACHE_DIR, "package.json"),
    JSON.stringify({ dependencies: { "oh-my-magento": "latest", other: "1.0.0" } }, null, 2)
  )
  writeFileSync(
    ...
    JSON.stringify(
      {
        workspaces: {
          "": {
            dependencies: { "oh-my-magento": "latest", other: "1.0.0" },
          },
        },
        packages: {
          "oh-my-magento": {},
  ...
  writeFileSync(
    join(TEST_OPENCODE_CACHE_DIR, "node_modules", "oh-my-magento", "package.json"),
    '{"name":"oh-my-magento"}'
  )
}
...
describe("invalidatePackage", () => {
  ...
  it("invalidates the installed package from the OpenCode cache directory", async () => {
    ...
    expect(existsSync(join(TEST_OPENCODE_CACHE_DIR, "node_modules", "oh-my-magento"))).toBe(false)

    const packageJson = JSON.parse(readFileSync(join(TEST_OPENCODE_CACHE_DIR, "package.json"), "utf-8")) as {
      dependencies?: Record<string, string>
    }
    expect(packageJson.dependencies?.["oh-my-magento"]).toBe("latest")
    ...
    expect(bunLock.workspaces?.[""]?.dependencies?.["oh-my-magento"]).toBe("latest")
    expect(bunLock.workspaces?.[""]?.dependencies?.other).toBe("1.0.0")
    expect(bunLock.packages?.["oh-my-magento"]).toBeUndefined()
