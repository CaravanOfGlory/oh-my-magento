/**
 * Detects external plugins that may conflict with oh-my-magento features.
...
/**
 * Known notification plugins that conflict with oh-my-magento's session-notification.
...
export function getNotificationConflictWarning(pluginName: string): string {
  return `[oh-my-magento] External notification plugin detected: ${pluginName}

Both oh-my-magento and ${pluginName} listen to session.idle events.
   Running both simultaneously can cause crashes on Windows.

   oh-my-magento's session-notification has been auto-disabled.
