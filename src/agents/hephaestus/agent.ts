...
function buildDynamicHephaestusPrompt(ctx?: HephaestusContext): string {
  ...
  const agentIdentity = buildAgentIdentitySection(
    "Hephaestus",
    "Autonomous deep worker for software engineering from OhMyMagento",
  );
...
export function createHephaestusAgent(
  ...
  return {
    description:
      "Autonomous Deep Worker - goal-oriented execution with GPT Codex. Explores thoroughly before acting, uses explore/librarian agents for comprehensive context, completes tasks end-to-end. Inspired by AmpCode deep mode. (Hephaestus - OhMyMagento)",
    mode: MODE,
    model,
    maxTokens: 32000,
    prompt,
