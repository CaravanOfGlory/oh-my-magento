...
export async function checkAndInterruptStaleTasks(args: {
  ...
  for (const task of tasks) {
    ...
    if (!task.progress?.lastUpdate) {
      ...
      task.error = `Stale timeout (no activity for ${staleMinutes}min since start). This is a FINAL cancellation - do NOT create a replacement task. If the timeout is too short, increase 'background_task.staleTimeoutMs' in .opencode/oh-my-magento.json.`
    ...
    if (task.status !== "running") continue
     ...
     task.status = "cancelled"
     task.error = `Stale timeout (no activity for ${staleMinutes}min). This is a FINAL cancellation - do NOT create a replacement task. If the timeout is too short, increase 'background_task.staleTimeoutMs' in .opencode/oh-my-magento.json.`
     task.completedAt = new Date()
