...
describe("discoverInstalledPlugins", () => {
  ...
  it("derives package name from file URL plugin keys", () => {
    //#given
    const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
    const installPath = join(createTemporaryDirectory("omo-plugin-install-"), "oh-my-magento")
    mkdirSync(installPath, { recursive: true })
    ...
    writeFileSync(
      ...
      JSON.stringify({
        version: 2,
        plugins: {
          "file:///D:/configs/user-configs/.config/opencode/node_modules/oh-my-magento@latest": [
            {
              scope: "user",
              installPath,
              version: "3.10.0",
    ...
    expect(discovered.errors).toHaveLength(0)
    expect(discovered.plugins).toHaveLength(1)
    expect(discovered.plugins[0]?.name).toBe("oh-my-magento")
  })
