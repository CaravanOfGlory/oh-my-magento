...
export async function checkSystem(deps: SystemCheckDeps = defaultDeps): Promise<CheckResult> {
  ...
  if (!pluginInfo.registered) {
    issues.push({
      title: `${OPENCODE_PLUGIN_NAME} is not registered`,
  ...
  if (pluginInfo.entry && !pluginInfo.isLocalDev) {
    const isLegacyName = pluginInfo.entry === LEGACY_PLUGIN_NAME
      ...
      || pluginInfo.entry === "oh-my-opencode"
      || pluginInfo.entry.startsWith("oh-my-opencode@")

    if (isLegacyName) {
      const suggestedEntry = pluginInfo.entry
        .replace(LEGACY_PLUGIN_NAME, OPENCODE_PLUGIN_NAME)
        .replace("oh-my-opencode", OPENCODE_PLUGIN_NAME)
      issues.push({
        title: "Using legacy package name",
        description: `Your opencode.json references a legacy plugin entry which has been renamed to "${OPENCODE_PLUGIN_NAME}". The old name may stop working in a future release.`,
  ...
  if (
    ...
    issues.push({
      ...
      fix: `Update: cd "${loadedInfo.cacheDir}" && bun add ${loadedInfo.installedPackageName}@${installTag}`,
      severity: "warning",
