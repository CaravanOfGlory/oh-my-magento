...
describe("addPluginToOpenCodeConfig - single package writes", () => {
  ...
  it("updates a version-pinned legacy entry to the requested version", async () => {
    // given
    const getPluginNameWithVersionSpy = spyOn(pluginNameWithVersion, "getPluginNameWithVersion").mockResolvedValue("oh-my-magento@3.16.0")
    writeFileSync(testConfigPath, JSON.stringify({ plugin: ["oh-my-openagent@3.15.0"] }, null, 2) + "\n", "utf-8")
    ...
    expect(result.success).toBe(true)
    const savedConfig = JSON.parse(readFileSync(testConfigPath, "utf-8"))
    expect(savedConfig.plugin).toEqual(["oh-my-magento@3.16.0"])
  ...
  it("preserves a canonical entry when the same version is re-installed", async () => {
    // given
    const getPluginNameWithVersionSpy = spyOn(pluginNameWithVersion, "getPluginNameWithVersion").mockResolvedValue("oh-my-magento@3.10.0")
  ...
  it("blocks a downgrade for a version-pinned canonical entry", async () => {
    // given
    const getPluginNameWithVersionSpy = spyOn(pluginNameWithVersion, "getPluginNameWithVersion").mockResolvedValue("oh-my-magento@3.15.0")
    writeFileSync(testConfigPath, JSON.stringify({ plugin: ["oh-my-magento@3.16.0"] }, null, 2) + "\n", "utf-8")
