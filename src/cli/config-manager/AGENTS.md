...
| File | Purpose |
|------|---------|
| `add-plugin-to-opencode-config.ts` | Register `oh-my-magento` in `.opencode/opencode.json` plugin array |
...
| `generate-omo-config.ts` | Generate `oh-my-magento.jsonc` from install selections |
...
| `plugin-name-with-version.ts` | Resolve `oh-my-magento@X.Y.Z` for installation |
| `write-omo-config.ts` | Write generated config to `.opencode/oh-my-magento.jsonc` |
