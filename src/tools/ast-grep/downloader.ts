...
export async function downloadAstGrep(version: string = DEFAULT_VERSION): Promise<string | null> {
  ...
  if (!platformInfo) {
    log(`[oh-my-magento] Unsupported platform for ast-grep: ${platformKey}`)
    return null
  }
  ...
  log(`[oh-my-magento] Downloading ast-grep binary...`)
  ...
  try {
    ...
    log(`[oh-my-magento] ast-grep binary ready.`)

    return binaryPath
  } catch (err) {
    log(
      `[oh-my-magento] Failed to download ast-grep: ${err instanceof Error ? err.message : err}`
    )
    return null
  }
}
