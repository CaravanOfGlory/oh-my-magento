...
```bash
bunx oh-my-magento install  # 推奨
npx oh-my-magento install   # 代替
```

> **注意**: CLIはすべての主要プラットフォーム用のスタンドアロンバイナリを同梱しています。インストール後、CLI実行にランタイム（Bun/Node.js）は不要です。
>
> **サポートプラットフォーム**: macOS (ARM64, x64)、Linux (x64, ARM64, Alpine/musl)、Windows (x64)
